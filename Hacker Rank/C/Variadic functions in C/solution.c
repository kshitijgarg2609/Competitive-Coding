...
int  sum (int count,...) {
    ...
    va_start(arg, count);
    int total=0;
    for(int i=0;i<count;total+=va_arg(arg,int),i++);
    va_end(arg);
    return total;
}
...
int min(int count,...) {
    ...
    int val;
    for(int i=1;i<count;val=va_arg(arg,int),min=((val<min)?val:min),i++);
    va_end(arg);
    return min;
}
...
int max(int count,...) {
    ...
    int val;
    for(int i=1;i<count;val=va_arg(arg,int),max=((val>max)?val:max),i++);
    va_end(arg);
    return max;
}
